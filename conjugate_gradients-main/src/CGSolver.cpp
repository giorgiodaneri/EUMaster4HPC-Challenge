...
{
    // y = alpha * x + beta * y
    // #pragma omp target map(to : size) map(tofrom : x)
    // #pragma omp teams num_teams(4) thread_limit(size / 2)
...
double CGSolver::dot_acc(const double *x, const double *y, size_t size)
{
    double result = 0.0;
...
void CGSolver::axpby_acc(double alpha, const double *x, double beta, double *y, size_t size)
{
// y = alpha * x + beta * y
...
void CGSolver::precA_acc(const double *A, const double *x, double *Ax, size_t size)
{
...
// #pragma acc parallel loop vector copyin(b[0 : size]) copyout(x[0 : size], r[0 : size], p[0 : size])
    ...
    for (num_iters = 1; num_iters <= max_iters; num_iters++)
    {
        ...
        // is this some kind of obfuscation to make the code less readable???
        // gemv(1.0, A, p, 0.0, Ap, size, size);
        precA_acc(A, p, Ap, size);
        // compute new alpha coefficient to guarantee optimal convergence rate
        alpha = rr / dot_acc(p, Ap, size);

        // compute new approximate of the solution at step k+1
        // x_k+1 = x_k + alpha_k * p_k
        axpby_acc(alpha, p, 1.0, x, size);

        // compute new residual at step k+1
        // r_k+1 = r_k - alpha_k * A * p_k
        axpby_acc(-alpha, Ap, 1.0, r, size);

        // update the 2-norm of the residual at step k+1
        rr_new = dot_acc(r, r, size);
        // compute beta coefficient
        // beta_k = ||r_k+1||^2 / ||r_k||^2
        // stopping criterion ==> sqrt(||r||^2 / ||b||^2) < rel_error equivalent to 2-norm or euclidean norm
        beta = rr_new / rr;

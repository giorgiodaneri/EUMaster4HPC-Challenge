...
void CGSolverACC::solve_acc()
{
    ...
    for (num_iters = 1; num_iters <= max_iters; num_iters++)
    {
#pragma acc data copyin(p[0 : size]) copyout(Ap[0 : size])
#pragma acc parallel loop gang vector present(A[0 : size * size])
        for (size_t i = 0; i < size; i++)
        {
            double y_val = 0.0;
            for (size_t j = 0; j < size; j++)
    ...
    // #pragma acc exit data delete (A[0 : size * size], Ap[0 : size], p[0 : size])
    // #pragma acc exit data delete (this[0 : 1])
